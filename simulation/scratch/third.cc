...
uint32_t pbt_marking_interval = 6;
uint32_t pbt_marking_offset = 6;
uint32_t pbt_min = 0;
uint32_t pbt_max = 1073741824;
...
uint32_t qlen_dump_interval = 10000000, qlen_mon_interval = 100;
...
void qp_finish(FILE* fout, Ptr<RdmaQueuePair> q){
	...
	completed_flow_num++;
	if (completed_flow_num == flow_num) {
		for (uint32_t i = 0; i < n.GetN(); i++){
			if (n.Get(i)->GetNodeType() == 1){ // is switch
				Ptr<SwitchNode> sw = DynamicCast<SwitchNode>(n.Get(i));
				Simulator::Cancel(sw->slotResetEvent);
			}
		}
	}
}
...
void get_pbt(FILE* fout, Ptr<RdmaQueuePair> q, CustomHeader ch, uint64_t old_rate, uint64_t new_rate){
	uint32_t sid = ip_to_node_id(q->sip), did = ip_to_node_id(q->dip);
	// sip, dip, sport, dport, size (B), start_time, time_from_start (ns), 
	fprintf(fout, "%u %u %u %u %lu %lu %lu\n", sid, did, q->sport, q->dport, q->m_size, q->startTime.GetTimeStep(), (Simulator::Now() - q->startTime).GetTimeStep());
	fprintf(fout, "    Postcard: %u %u %u %u %u %lu %.3lf->%.3lf\n", ch.pbt.switchID, ch.pbt.rxBytes, ch.pbt.queueSize, ch.pbt.recentRxBytes, ch.pbt.recentMarkedRxBytes, ch.pbt.linkBw, old_rate*1e-9, new_rate*1e-9);
	fflush(fout);
}
...
int main(int argc, char *argv[])
{
...
#endif
	{
		...
		while (!conf.eof())
		{
			...
			else if (key.compare("ENABLE_PBT") == 0)
			{
				uint32_t v;
				conf >> v;
				enable_pbt = v;
				if (enable_pbt)
					std::cout << "ENABLE_PBT\t\t\t" << "Yes" << "\n";
				else
					std::cout << "ENABLE_PBT\t\t\t" << "No" << "\n";
			}
			...
			}else if (key.compare("PBT_MARKING_INTERVAL") == 0){
				conf >> pbt_marking_interval;
				std::cout << "PBT_MARKING_INTERVAL\t\t\t\t" << pbt_marking_interval << '\n';
			}else if (key.compare("PBT_MARKING_OFFSET") == 0){
				conf >> pbt_marking_offset;
				std::cout << "PBT_MARKING_OFFSET\t\t\t\t" << pbt_marking_offset << '\n';
			}else if (key.compare("PBT_MIN") == 0){
				conf >> pbt_min;
				std::cout << "PBT_MIN\t\t\t\t" << pbt_min << '\n';
			}else if (key.compare("PBT_MAX") == 0){
				conf >> pbt_max;
				std::cout << "PBT_MAX\t\t\t\t" << pbt_max << '\n';
			}
			fflush(stdout);
		}
		conf.close();
	...
	NS_ASSERT_MSG(pbt_marking_interval >= pbt_marking_offset, "marking interval must be large or equal to marking offset");
	...
	for (uint32_t i = 0; i < node_num; i++){
		if (n.Get(i)->GetNodeType() == 0){ // is server
			...
			rdmaHw->SetAttribute("PbtMarkInterval", UintegerValue(pbt_marking_interval));
